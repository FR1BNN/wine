...
def check():
    if os.system("wine --version")==0:
        label1.configure(text="Found", foreground="#00FF00")
        Start.config(state=["enabled"])
        warn.destroy()
        copy_button.destroy()
    else:
        label1.configure(text="Not found", foreground="#FF0000")
...
def wine():
    ...
    label = ttk.Label(text="Wine launched, enjoy" ,font=("Arial", 16), background="#3F3F3F", foreground="#FFFFFF")
    label.pack(expand=True)

    tk1.after(1500,tk1.destroy)
    

btn = ttk.Button(text="Check",command=check)
btn.place(x=410, y=20)

label = ttk.Label(text="Wine:", font=("Arial", 18), background="#3F3F3F", foreground="#FFFFFF")
label.place(x=170,y=20)

label1 = ttk.Label(text="Not found", font=("Arial", 18), background="#3F3F3F", foreground="#FF0000")
label1.place(x=250,y=20)

warn = ttk.Label(text="!To install Wine click here ->", font=("Arial", 11), background="#3F3F3F", foreground="#C5C5C5")
warn.place(x=180,y=70)

copy_button=ttk.Button(text="Copy command",command=lambda: pc.copy("sudo apt install wine"))
copy_button.place(x=400,y=65)

Start=ttk.Button(text="Start Wine",command=wine,state=["disabled"])
